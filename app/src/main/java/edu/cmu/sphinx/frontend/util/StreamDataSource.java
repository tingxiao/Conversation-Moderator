    ...
    public Data getData() throws DataProcessingException {
        ...
        if (streamEndReached) {
            ...
            if (!utteranceStarted) {
                ...
                if (dataStream != null) {
                   // System.out.println("*******************************************it is going to read the dataStream and the sampleRate is "+sampleRate);
        ...
        try {
            do {
                read = dataStream.read(samplesBuffer, totalRead, bytesToRead
                        - totalRead);
              //  System.out.println("*******************************the total value read for now is "+read);
                if (read > 0) {
                    totalRead += read;
                }
            ...
            if (totalRead < bytesToRead) {
                ...
                System
                        .arraycopy(samplesBuffer, 0, shrinkedBuffer, 0,
                                totalRead);
               // System.out.println("************************************************the arrayValue that is read is "+samplesBuffer);
